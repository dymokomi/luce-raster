#!/usr/bin/env python3
"""Build and run luce-raster's test blocks in native and C modes."""
...
for source in ["src/luce_raster/model.lucb", "src/luce_raster/binary.lucb", "src/luce_raster/errors.lucb"]:
    for flags in MODES:
        subprocess.run([str(BASE.resolve()), "test", str(ROOT / source), *flags], env=env, check=True, timeout=300, cwd=ROOT)
print("PASS luce-raster")
